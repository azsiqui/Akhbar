...
import { BookOpen, Upload, CheckCircle2, Clock, Calendar, Sparkles } from 'lucide-react';
...
export default function BrandNewspaperCard({ paper, onOpenReader, onUploadPaper, onToggleComplete }) {
  ...
  const progressPercent = Math.round(((paper?.readPage || 0) / (paper?.pageCount || 1)) * 100);
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        {paper.editorialSnippet && (
          <div className="p-3 rounded-xl bg-cream-50 dark:bg-brown-900/40 border border-brown-200/40 dark:border-brown-800">
            <span className="text-[10px] font-bold uppercase tracking-wider text-gold-700 dark:text-gold-400 font-sans block mb-1">
              Editorial Highlight &bull; {paper.date}
            </span>
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          ...
          <button
            ...
            className="py-2.5 px-3 rounded-xl bg-brown-500 hover:bg-brown-600 dark:bg-gold-400 dark:hover:bg-gold-500 text-cream-100 dark:text-brown-950 text-xs font-bold flex items-center justify-center gap-1.5 shadow-xs transition-all active:scale-95"
          >
            <BookOpen className="w-4 h-4" />
            <span>Read PDF</span>
          </button>

          {/* Direct 1-Click Upload Button (No Title Renaming Needed!) */}
          <button
